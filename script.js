    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (data.group) {
          ...
          groupBox.querySelector(".group-name").innerHTML = `<span class="label">Name:</span> <span class="value">${data.group.name}</span>`;
          groupBox.querySelector(".group-level").innerHTML = `<span class="label">Level:</span> <span class="value">${data.group.level}</span>`;
          groupBox.querySelector(".group-group").innerHTML = `<span class="label">Group:</span> <span class="value">${data.group.group}</span>`;
          groupBox.querySelector(".group-schedule").innerHTML = `<span class="label">Schedule:</span> <span class="value">${data.group.schedule}</span>`;
          groupBox.querySelector(".group-location").innerHTML = `<span class="label">Location:</span> <span class="value">${data.group.location}</span>`;
          groupBox.querySelector(".group-teacher").innerHTML = `<span class="label">Professor:</span> <span class="value">${data.group.professor}</span>`;
          const mapBtn = groupBox.querySelector(".map-btn");
          const mapImg = groupBox.querySelector("#group-map-img");
          if (data.group.map) {
            mapBtn.style.display = "inline-block";
            mapImg.src = data.group.map;
